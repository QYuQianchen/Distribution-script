...
impl Requirements {
    ...
    pub fn new(
        ...
        Ok(Self {
            min_safe_balance: parse_units(&min_safe_balance_in_eth, "ether")?.into(),
            min_running_node,
        })
    }
    ...
    pub fn check_requirements(&self, safe_balance: &U256, running_nodes: &u32) -> bool {
        ...
        debug!(
            "Checking requirements: balance: {:?}, nodes: {:?}",
...
impl Claims {
    ...
    pub fn check_claimed(&self, safes: &[Address]) -> bool {
        safes
            .iter()
            .any(|safe| self.claims.iter().any(|claim| claim.safe == *safe))
    }
}
    ...
    for owner in owners {
        ...
        if let Some(owned_assets) = asset_list.asset_list.get(owner) {
            ...
            if requirements.check_at_least_one_asset_fulfills_requirments(owned_assets) {
                ...
                let claimed_safes = owned_assets
                    .safes
                    .iter()
                    .map(|safe| Claim {
            ...
            debug!(
                "Owner: {:?} is not eligible for claim because it does not have any safes",
                owner
            );
            continue;
        }
...
mod tests {
    ...
    fn test_requirements() {
        init();
        let requirements = Requirements::new("32".to_string(), 3).unwrap();

        assert!(
            requirements.check_requirements(&U256::from_str("32000000000000000000").unwrap(), &3)
        );
        assert!(
            !requirements.check_requirements(&U256::from_str("31000000000000000000").unwrap(), &3)
        );
        assert!(
            !requirements.check_requirements(&U256::from_str("32000000000000000000").unwrap(), &2)
        );
    }
    ...
    fn test_check_owners_eligibility_account_a_first() {
        ...
        let requirements = Requirements::new("10000000".to_string(), 1).unwrap();
        ...
        let asset_a = Assets {
            ...
            nodes_count: vec![1, 2],
            safes: vec![
                address!("0d9d6d05a37353a98a9beaaf2c852089793f5dd1"),
                address!("0bb7ac4a34a92d1b417b94dd8866e5b90e544c80"),
            ],
        ...
        let eligible_account =
            check_owners_eligibility(&owners, &requirements, &asset_list, &mut claim_history);
